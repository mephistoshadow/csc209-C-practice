...
struct pixel **read_pixel_array(FILE *image, int pixel_array_offset, int width, int height) {
	...
	struct pixel ** pixel = malloc(sizeof(struct pixel*) * height);
	for (int i =0; i <height; i ++) {
		pixel[i] = malloc(sizeof(struct pixel)* width);
	}
	for (int i =0; i < height; i++) {
		for (int j = 0; j < width;j ++ ) {

			fread(&pixel[i][j].blue,sizeof(int)/4,1,image);
			fread(&pixel[i][j].green,sizeof(int)/4,1,image);
			fread(&pixel[i][j].red,sizeof(int)/4,1,image);
		}
	}
